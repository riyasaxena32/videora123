...
function VideoCard({ video, onClick, onDelete, isCurrentUser }) {
  ...
  const checkIfVideoInWatchLater = async () => {
    ...
    try {
      ...
      const isInWatchLater = watchLaterVideos.some(watchVideo => watchVideo._id === video._id);
  ...
  const handleSaveVideo = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to update saved video');
      }

      const data = await response.json();
      console.log('Saved video status updated:', data.message);
      
      // Toggle the saved state
      setIsSaved(!isSaved);
    } catch (err) {
      console.error('Error updating saved video status:', err);
    }
      ...
      try {
        ...
        const uniqueCreators = Array.from(new Set(videos.map(v => v.uploadedBy)))
          ...
          .map(creator => {
            ...
            const creatorVideos = videos.filter(v => v.uploadedBy === creator);
            ...
            return {
              ...
              isCurrentUser: user && creator === user.name
            };
          });
        ...
        // Regular creator profile
          ...
          if (apiCreator) {
            ...
            // Set following state
            setIsFollowing(apiCreator.isFollowing || false);
          }
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    {!isCurrentUserProfile ? (
                      <button 
                        ...
                        style={{
                          backgroundColor: isFollowing ? '#333333' : 'transparent',
                          borderColor: isFollowing ? '#555555' : 'rgba(255, 255, 255, 0.2)'
                        }}
                      >
